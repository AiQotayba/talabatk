...
export default function AdminProfileScreen() {
  ...
  const handleLogout = () => {
    showDialog({
      ...
      type: 'warning',
      onConfirm: () => dispatch(logout() as any).then(() => router.push(ROUTES.LOGIN)),
    });
  };

  return (
    <ScrollView className="flex-1 bg-gray-50 mt-8" showsVerticalScrollIndicator={false} style={{ direction: 'rtl' }}>
      <View className="bg-white px-6 pb-8 pt-12" style={{ direction: 'rtl' }}>
      ...
      <View className="px-6 mt-4" style={{ direction: 'rtl' }}>
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(150)}>
          ...
          >
            <View className="flex-row items-center gap-3">
              <Ionicons name="settings-outline" size={24} color="#E02020" />
              <Text className="text-gray-900 font-semibold text-base text-right">الإعدادات</Text>
            </View>
            <Ionicons name="chevron-back" size={20} color="#9ca3af" />
          </TouchableOpacity>
        </Animated.View>
