...
type UpdateBannerFormData = z.infer<typeof updateBannerSchema>;
...
export default function EditBannerScreen() {
  ...
  const { data: banner, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<Banner>(`/admin/banner/${id}`);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: UpdateBannerFormData) => {
      const response = await apiClient.put(`/admin/banner/${id}`, {
    ...
    onSuccess: () => {
      ...
      Toast.success('تم التحديث بنجاح', 'تم تحديث البانر بنجاح! سيظهر التحديث للعملاء فوراً');
      router.back();
    },
    onError: (error: any) => {
      Toast.error('حدث خطأ', error.message || 'حدث خطأ أثناء تحديث البانر. يرجى التحقق من البيانات والمحاولة مرة أخرى');
    },
  });

  const onSubmit = (data: UpdateBannerFormData) => {
  ...
  if (isLoading) {
    return (
      <View className="flex-1 items-center justify-center bg-gray-50">
        ...
        <Text className="text-gray-500 mt-4 text-start">جاري تحميل بيانات البانر...</Text>
      </View>
    );
  }
  ...
  if (!banner) {
    return (
      <View className="flex-1 items-center justify-center bg-gray-50">
        ...
        <Text className="text-gray-500 mt-4 text-start">البانر غير موجود</Text>
        <Text className="text-gray-400 text-sm mt-2 text-start">يرجى العودة والتحقق من البانر المطلوب</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView
      ...
      style={{ direction: 'rtl' }}
    >
      <Header title="تعديل البانر" description="قم بتحديث معلومات البانر. سيظهر التعديل للعملاء فوراً بعد الحفظ" />
          ...
          >
            {updateMutation.isPending ? (
              ...
              <View className="flex-row items-center justify-center gap-2">
                ...
                <Text className="text-white font-bold text-lg">تحديث البانر</Text>
              </View>
            )}
          </TouchableOpacity>
        </Animated.View>
